  ...
  <div>
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ filteredPeople.length }} people</p>
      </div>
      <div class="w-72">
        <input
          v-model="searchQuery"
          ...
          class="w-full text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-primary-500 focus:border-primary-500"
        />
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4 mb-6 flex flex-wrap items-start gap-6">
        ...
        <div class="space-y-1 max-h-48 overflow-y-auto">
          ...
          >
            <input
              ...
              class="rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ team.displayName }} ({{ team.members.length }})</span>
          </label>
        </div>
      </div>
      ...
      <div v-if="primaryDisplayField && availablePrimaryValues.length > 0">
        <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 uppercase mb-1">{{ primaryFieldLabel }}</label>
        <div class="space-y-1 max-h-48 overflow-y-auto">
          <label
          ...
          >
            <input
              ...
              class="rounded border-gray-300 dark:border-gray-600 text-primary-600 focus:ring-primary-500"
            />
            <span class="text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">{{ val }}</span>
          </label>
        </div>
      </div>
          ...
          >
            <td class="px-4 py-2 text-sm whitespace-nowrap">
              <button
                ...
                class="text-primary-600 hover:text-primary-800 dark:hover:text-primary-400 font-medium hover:underline text-left"
              >{{ person.name }}</button>
            </td>
            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">{{ person.orgName }}</td>
            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">{{ person.teamName || '—' }}</td>
            ...
            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
              <span v-if="person.metrics?.nameNotFound" class="text-gray-400 dark:text-gray-500 italic">no Jira user</span>
            ...
            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
              ...
              <span v-else-if="person.githubUsername" class="text-gray-300 dark:text-gray-600">—</span>
            ...
            <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
              ...
              <span v-else-if="person.gitlabUsername" class="text-gray-300 dark:text-gray-600">—</span>
