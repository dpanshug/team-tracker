<template>
  <div>
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
      <button @click="nav.navigateTo('dashboard')" class="hover:text-primary-600 dark:hover:text-primary-400 transition-colors">Dashboard</button>
      <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
      </svg>
      <button @click="nav.goBack()" class="hover:text-primary-600 dark:hover:text-primary-400 transition-colors">{{ teamName }}</button>
      <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
      </svg>
      <span class="text-gray-900 dark:text-gray-100 font-medium">{{ person.name }}</span>
    </nav>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-5 mb-6">
      <div class="flex items-start justify-between">
        <div>
          ...
          <div class="flex items-center gap-x-4 gap-y-1 flex-wrap text-sm text-gray-500 dark:text-gray-400 mb-2">
            <span v-if="person.manager">
              <span class="font-medium text-gray-700 dark:text-gray-300">Manager:</span> {{ person.manager }}
            </span>
            <template v-if="person.customFields">
              <span
                ...
                :key="field.key"
              >
                <span class="font-medium text-gray-700 dark:text-gray-300">{{ field.label }}:</span> {{ person.customFields[field.key] || '—' }}
              </span>
            </template>
          </div>
    ...
    <!-- Stale data warning -->
    <div v-if="metrics?.stale" class="bg-amber-50 dark:bg-amber-900/20 border border-amber-300 dark:border-amber-700 rounded-lg p-4 mb-6 flex items-start gap-3">
      ...
      <div>
        <p class="text-amber-800 dark:text-amber-300 text-sm font-medium">Showing cached data</p>
    ...
    <div v-if="metrics?._nameNotFound" class="bg-orange-50 dark:bg-orange-900/20 border border-orange-300 dark:border-orange-700 rounded-lg p-4 mb-6 flex items-start gap-3">
      ...
      <div>
        <p class="text-orange-800 dark:text-orange-300 text-sm font-medium">No matching Jira account found</p>
    ...
    <template v-else-if="metrics">
      ...
      <!-- Fetched timestamp -->
      <p v-if="metrics.fetchedAt" class="text-xs text-gray-400 dark:text-gray-500 mb-4">
        Data fetched: {{ new Date(metrics.fetchedAt).toLocaleString() }}
      </p>

      <!-- Resolved Issues Table -->
      <div v-if="metrics.resolved.issues.length > 0" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 mb-6">
        <h3 class="px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
          Resolved Issues ({{ metrics.resolved.issues.length }})
          <span class="font-normal text-gray-400 dark:text-gray-500 ml-1">— last {{ metrics.lookbackDays }} days</span>
        </h3>
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="issue in metrics.resolved.issues" :key="issue.key" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
      ...
      <div v-if="metrics.inProgress.issues.length > 0" class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
        <h3 class="px-4 py-3 text-sm font-semibold text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700">
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="issue in metrics.inProgress.issues" :key="issue.key" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td class="px-4 py-2 text-sm">
                  <a
                    :href="`https://redhat.atlassian.net/browse/${issue.key}`"
                    target="_blank"
