        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex gap-2">
              <button @click="selectAll" class="text-xs text-primary-600 hover:text-primary-800">Select All</button>
              <span class="text-gray-300">|</span>
              <button @click="deselectAll" class="text-xs text-primary-600 hover:text-primary-800">Deselect All</button>
            </div>
          </div>
...
const canSave = computed(() =>
  name.value.trim().length > 0 && selectedBoardIds.value.length > 0
)

function selectAll() { selectedBoardIds.value = props.boards.map(b => b.id) }
function deselectAll() { selectedBoardIds.value = [] }
