<template>
  <div class="relative">
    ...
    >
      ...
      <div v-for="filter in filters" :key="filter.id" class="group flex items-center">
        <button
        ...
        <div class="flex items-center gap-1 pr-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button @click.stop="$emit('edit-filter', filter.id)" class="p-1 text-gray-400 hover:text-gray-600 rounded" title="Edit">
            <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
          </button>
          <button @click.stop="$emit('delete-filter', filter.id)" class="p-1 text-gray-400 hover:text-red-500 rounded" title="Delete">
            <svg class="h-3.5 w-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
